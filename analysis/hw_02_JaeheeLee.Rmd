...
-Extra Credit: Test for a linear relationship using lm() and interpret the results in one sentence based on the p
-value and adjusted R-squared.
```{r}
# - Create an appropriate plot. 
library(ggplot2)
library(ggthemes)
ggplot(data = civil_war_with_days, aes(y = days , x =factor(CWSAC)))+
  geom_boxplot(color="orange", fill="yellow", alpha=0.2)+ 
  xlab("CWSAC")+
  ylab("Days")+
  theme_classic()
# - Interpret the plot in one sentence to answer the question.
# When 'A rating of the military significance of the battle by the Civil War Sites Advisory Commission' is `A` = #Decisive,the days of the battle spanned have the highest median compared to other `B` = Major, `C` = Formative, `D` = #Limited

# -Extra Credit: Test for a linear relationship using lm() and interpret the results in one sentence based on the p
# -value and adjusted R-squared.
fit <- lm(days ~ factor(CWSAC), data = civil_war_with_days)
fit
summary(fit)
```

10. Extra Credit: Did the theaters of war shift during the American Civil War?
-Reproduce this plot in R (filter out states with two or fewer battles.)
-Interpret the plot in one sentence.
```{r}
# Basic box plot
ggplot(civil_war_with_days, aes(x=State, y=Start, fill = factor(Theater)))+
  geom_boxplot()+ 
  coord_flip()
```
